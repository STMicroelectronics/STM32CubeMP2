                                    ...
                                    uint32_t BufferSize)
{
  ...
  if ((state == HAL_SRAM_STATE_READY) || (state == HAL_SRAM_STATE_PROTECTED))
  {
    ...
    if (hsram->hdma->Init.DestDataWidth == DMA_DEST_DATAWIDTH_WORD)
    {
      status = HAL_DMA_Start_IT(hsram->hdma, (uint32_t)pAddress, (uint32_t)pDstBuffer, (uint32_t)(BufferSize * 4U));
    }
    else if (hsram->hdma->Init.DestDataWidth == DMA_DEST_DATAWIDTH_HALFWORD)
    {
      status = HAL_DMA_Start_IT(hsram->hdma, (uint32_t)pAddress, (uint32_t)pDstBuffer, (uint32_t)(BufferSize * 2U));
    }
    else
    {
                                     ...
                                     uint32_t BufferSize)
{
  ...
  if (hsram->State == HAL_SRAM_STATE_READY)
  {
    ...
    if (hsram->hdma->Init.DestDataWidth == DMA_DEST_DATAWIDTH_WORD)
    {
      status = HAL_DMA_Start_IT(hsram->hdma, (uint32_t)pSrcBuffer, (uint32_t)pAddress, (uint32_t)(BufferSize * 4U));
    }
    else if (hsram->hdma->Init.DestDataWidth == DMA_DEST_DATAWIDTH_HALFWORD)
    {
      status = HAL_DMA_Start_IT(hsram->hdma, (uint32_t)pSrcBuffer, (uint32_t)pAddress, (uint32_t)(BufferSize * 2U));
    }
    else
    {
