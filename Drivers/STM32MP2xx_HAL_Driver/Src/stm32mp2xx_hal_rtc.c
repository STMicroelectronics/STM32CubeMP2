...
HAL_StatusTypeDef HAL_RTC_SetAlarm(RTC_HandleTypeDef *hrtc, RTC_AlarmTypeDef *sAlarm, uint32_t Format)
{
  ...
  if (sAlarm->Alarm == RTC_ALARM_A)
  {
    ...
    if (sAlarm->FlagAutoClr == ALARM_FLAG_AUTOCLR_ENABLE)
    {
      ...
      SET_BIT(RTC->CR, RTC_CR_ALRACLR);
    }
    else
    {
      /* Disable the  Alarm A  output clear */
      CLEAR_BIT(RTC->CR, RTC_CR_ALRACLR);
    }
  ...
  else
  {
    ...
    if (sAlarm->FlagAutoClr == ALARM_FLAG_AUTOCLR_ENABLE)
    {
      ...
      SET_BIT(RTC->CR, RTC_CR_ALRBCLR);
    }
    else
    {
      /* Disable the  Alarm B output clear */
      CLEAR_BIT(RTC->CR, RTC_CR_ALRBCLR);
    }
    /* Configure the Alarm state: Enable Alarm */
...
HAL_StatusTypeDef HAL_RTC_SetAlarm_IT(RTC_HandleTypeDef *hrtc, RTC_AlarmTypeDef *sAlarm, uint32_t Format)
{
  ...
  __HAL_RTC_ALARM_EXTI_ENABLE_IT(); /*exti should be in line 19*/
