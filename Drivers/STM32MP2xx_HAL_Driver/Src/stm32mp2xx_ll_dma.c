...
#define IS_LL_DMA_ALL_CHANNEL_INSTANCE(INSTANCE, Channel) ((((INSTANCE) == LPDMA)                 && \
...
#if defined(CORE_CM0PLUS)
#define IS_LL_HPDMA_CHANNEL_INSTANCE(INSTANCE, Channel)   0U
#else
...
#if  defined(I3C4)
#define IS_LL_DMA_REQUEST(REQUEST)     \
...
uint32_t LL_DMA_Init(DMA_TypeDef *DMAx, uint32_t Channel, LL_DMA_InitTypeDef *DMA_InitStruct)
{
  ...
  {
    assert_param(IS_LL_DMA_REQUEST(DMA_InitStruct->Request));
  }
  ...
  if ((DMAx == HPDMA1) || (DMAx == HPDMA2) || (DMAx == HPDMA3))
  {
...
uint32_t LL_DMA_CreateLinkNode(const LL_DMA_InitNodeTypeDef *DMA_InitNodeStruct, LL_DMA_LinkNodeTypeDef *pNode)
{
  ...
  {
    assert_param(IS_LL_DMA_REQUEST(DMA_InitNodeStruct->Request));
  }
