...
HAL_StatusTypeDef HAL_OSPI_Command(OSPI_HandleTypeDef *hospi, OSPI_RegularCmdTypeDef *cmd, uint32_t Timeout)
{
  ...
  if (((state == HAL_OSPI_STATE_READY) && (hospi->Init.MemoryType != HAL_OSPI_MEMTYPE_HYPERBUS)) ||
      ((state == HAL_OSPI_STATE_READ_CMD_CFG)  &&
       ((cmd->OperationType == HAL_OSPI_OPTYPE_WRITE_CFG) || (cmd->OperationType == HAL_OSPI_OPTYPE_WRAP_CFG))) ||
      ((state == HAL_OSPI_STATE_WRITE_CMD_CFG) &&
       ((cmd->OperationType == HAL_OSPI_OPTYPE_READ_CFG)  || (cmd->OperationType == HAL_OSPI_OPTYPE_WRAP_CFG))))
  {
    /* Wait till busy flag is reset */
...
HAL_StatusTypeDef HAL_OSPI_Transmit_DMA(OSPI_HandleTypeDef *hospi, uint8_t *pData)
{
  ...
  else
  {
    ...
    {
      /* Configure counters and size */
      if (hospi->hdma->Init.DestDataWidth == DMA_DEST_DATAWIDTH_BYTE)
      {
        hospi->XferCount = data_size;
      }
      else if (hospi->hdma->Init.DestDataWidth == DMA_DEST_DATAWIDTH_HALFWORD)
      {
      ...
      else if (hospi->hdma->Init.DestDataWidth == DMA_DEST_DATAWIDTH_WORD)
      {
      ...
      if (status == HAL_OK)
      {
        ...
        MODIFY_REG(hospi->hdma->Instance->CTR2, DMA_CTR2_DREQ, hospi->hdma->Init.Direction);
        hospi->hdma->Init.SrcInc = DMA_SINC_INCREMENTED;
        MODIFY_REG(hospi->hdma->Instance->CTR1, DMA_CTR1_SINC, hospi->hdma->Init.SrcInc);
        hospi->hdma->Init.DestInc = DMA_DINC_FIXED;
        MODIFY_REG(hospi->hdma->Instance->CTR1, DMA_CTR1_DINC, hospi->hdma->Init.DestInc);
  ...
  else
  {
    ...
    if (hospi->State == HAL_OSPI_STATE_CMD_CFG)
    {
      ...
      if (status == HAL_OK)
      {
        ...
        MODIFY_REG(hospi->hdma->Instance->CTR2, DMA_CTR2_DREQ, hospi->hdma->Init.Direction);
        hospi->hdma->Init.SrcInc = DMA_SINC_FIXED;
        MODIFY_REG(hospi->hdma->Instance->CTR1, DMA_CTR1_SINC, hospi->hdma->Init.SrcInc);
        hospi->hdma->Init.DestInc = DMA_DINC_INCREMENTED;
        MODIFY_REG(hospi->hdma->Instance->CTR1, DMA_CTR1_DINC, hospi->hdma->Init.DestInc);
...
HAL_StatusTypeDef HAL_OSPIM_Config(const OSPIM_CfgTypeDef *cfg, uint32_t Timeout)
{
  ...
  assert_param(IS_OSPIM_MUXEN(cfg->MuxEn));
  assert_param(IS_OSPIM_MODE(cfg->Mode));
  assert_param(IS_OSPIM_CSOVREN(cfg->CSOvrEn));
  assert_param(IS_OSPIM_CS1OVR(cfg->CS1Ovr));
  assert_param(IS_OSPIM_CS2OVR(cfg->CS2Ovr));
...
static HAL_StatusTypeDef OSPI_ConfigCmd(OSPI_HandleTypeDef *hospi, OSPI_RegularCmdTypeDef *cmd)
{
  ...
  if (hospi->Init.DualQuad == HAL_OSPI_DUALQUAD_DISABLE)
  {
    MODIFY_REG(hospi->Instance->CR, (OCTOSPI_CR_MSEL | (OCTOSPI_CR_MSEL << 23)), (cmd->FlashId | cmd->FlashId));
  }
