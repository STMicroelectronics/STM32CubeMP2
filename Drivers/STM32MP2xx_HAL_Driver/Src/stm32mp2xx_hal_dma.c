  ...
  ======================================================================================================================
    ...
    *** Valid burst length ***
    ...
    (+) The maximum allowed AXI burst length shall not exceed 16.
            (++) If either selected source source or destination length is above 16, HAL_DMA_Init will return HAL_ERROR.
...
HAL_StatusTypeDef HAL_DMA_Init(DMA_HandleTypeDef *const hdma)
{
  ...
  if ((ccid_cfen == DMA_CCIDCFGR_CFEN) && (ccid_semen == DMA_CCIDCFGR_SEMEN))
  {
    ...
    if (((READ_REG(hdma->Instance->CSEMCR) & DMA_CSEMCR_SEMCCID_Msk)) != DMA_CSEMCR_SEMCCID_CURRENT)
    {
...
HAL_StatusTypeDef HAL_DMA_ConfigChannelAttributes(DMA_HandleTypeDef *const hdma, uint32_t ChannelAttributes)
{
  ...
  /*shared CID field value used ONLY and CID filtering not disable */
  if ((ChannelAttributes & (DMA_CHANNEL_CID_DISABLE | DMA_CHANNEL_ATTR_CID_DYNAMIC_SELECT))
      == DMA_CHANNEL_ATTR_CID_DYNAMIC_SELECT)
  {
                                                     ...
                                                     uint32_t *const pChannelAttributes)
{
  ...
  {
    if ((hdma->Instance->CCIDCFGR & DMA_CCIDCFGR_SEMEN_Msk) == DMA_CCIDCFGR_SEMEN)
    {
      /* Get CIDs value from Semaphore white list */
      attributes |= ((DMA_CHANNEL_ATTR_CID_DYNAMIC_SELECT) | ((READ_REG(hdma->Instance->CCIDCFGR)) &
                                                              (DMA_CCIDCFGR_WLISTCID2_Msk |
                                                               DMA_CCIDCFGR_WLISTCID1_Msk |
                                                               DMA_CCIDCFGR_WLISTCID0_Msk)));
    }
    else
    {
...
/**
  * @brief  Attempt to acquire semaphore(s) of DMA Channel
  * @note   In case of semaphore acquisition failure, returned status is HAL_KO and semaphore acquisition is abandoned
  * @param  hdma : pointer to DMA Handler
  * @retval HAL Status, HAL_OK if semaphore acquisition is successful
  */
...
HAL_StatusTypeDef HAL_DMA_TakeChannelSemaphore(DMA_HandleTypeDef *hdma)
{
  ...
  /* Take Semaphore */
  hdma->Instance->CSEMCR |= DMA_CSEMCR_SEMMUTEX;
  if ((hdma->Instance->CSEMCR & DMA_CSEMCR_SEMCCID_Msk) != (cidcurrent << DMA_CSEMCR_SEMCCID_Pos))
  {
    /* Mutex not taken with current CID - it means that other authorized CID has control, previous acquired
       semaphore shall be released and status error is returned
    */
    (void)HAL_DMA_ReleaseChannelSemaphore(hdma);
...
/**
  * @brief  Release semaphore(s) of DMA Channel
  * @param  hdma : pointer to DMA Handler
...
static void DMA_Init(DMA_HandleTypeDef const *const hdma)
{
  ...
  /* Add parameters specific to HPDMA */
  if (IS_HPDMA_INSTANCE(hdma->Instance) != 0U)
  {
