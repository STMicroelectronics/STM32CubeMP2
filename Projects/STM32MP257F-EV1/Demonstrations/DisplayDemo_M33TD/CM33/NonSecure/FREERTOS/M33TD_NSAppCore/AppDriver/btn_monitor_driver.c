 ...
 * - BUTTON_USER2 on GPIOG pin 8
 * - EXTI2 line 8 (EXTI2_8_IRQn)
 */

/**
  * @brief EXTI handle for USER2 button (EXTI2 line 8).
  *
  * @note The EXTI2_8 IRQ handler is expected to call HAL_EXTI_IRQHandler(&hexti2).
  */
...
static int BtnMonitorDriver_Init(BtnMonitorDriver_IRQCallback cb, void *context)
{
  ...
  /* Enable GPIOG clock (RCC resource 96 in this demo) */
  ...
  /* Acquire EXTI2 line 8 */
  if (RESMGR_STATUS_ACCESS_OK != ResMgr_Request(RESMGR_RESOURCE_RIF_EXTI2, RESMGR_EXTI_RESOURCE(8)))
  {
  ...
  /* Configure EXTI2 line 8 rising edge on GPIOG */
...
static int BtnMonitorDriver_DeInit(void)
{
  ...
  (void)ResMgr_Release(RESMGR_RESOURCE_RIF_EXTI2, RESMGR_EXTI_RESOURCE(8));
  (void)ResMgr_Release(RESMGR_RESOURCE_RIF_GPIOG, RESMGR_GPIO_PIN(8));
