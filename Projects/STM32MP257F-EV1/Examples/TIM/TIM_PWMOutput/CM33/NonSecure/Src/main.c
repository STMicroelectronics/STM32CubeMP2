...
static void MX_TIM8_Init(void)
{
  ...
  /* Acquire GPIOJ3 using Resource manager */
  if (RESMGR_STATUS_ACCESS_OK != ResMgr_Request(RESMGR_RESOURCE_RIF_GPIOJ, RESMGR_GPIO_PIN(3)))
  {
	  Error_Handler();
  }

  /* Acquire GPIOJ4 using Resource manager */
...
static void MX_TIM8_DeInit(void)
{
    ...
    /* Release GPIOJ3 using Resource manager */
	ResMgr_Release(RESMGR_RESOURCE_RIF_GPIOJ, RESMGR_GPIO_PIN(3));

	/* Release GPIOJ4 using Resource manager */
	ResMgr_Release(RESMGR_RESOURCE_RIF_GPIOJ, RESMGR_GPIO_PIN(4));

	/* Release GPIOJ5 using Resource manager */
	ResMgr_Release(RESMGR_RESOURCE_RIF_GPIOJ, RESMGR_GPIO_PIN(5));

	/* Release TIM8 using Resource manager */
	ResMgr_Release(RESMGR_RESOURCE_RIFSC, STM32MP25_RIFSC_TIM8_ID);
