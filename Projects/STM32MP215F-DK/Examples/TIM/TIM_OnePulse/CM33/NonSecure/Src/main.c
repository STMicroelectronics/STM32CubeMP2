...
/**
  ******************************************************************************
  * @file    TIM\TIM_OnePulse\CM33\NonSecure\Src\main.c
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
  Prescaler = (HAL_RCCEx_GetTimerCLKFreq(RCC_PERIPHCLK_TIM5)/ (16 * 1000000)) - 1;
  ...
  if (HAL_TIM_OnePulse_Start(&htim5, TIM_CHANNEL_1) != HAL_OK)
  {
    /* Starting Error */
    Error_Handler();
  }
...
static void MX_TIM5_Init(void)
{
   ...
   *
   *-The external signal is connected to TIM5_CH2 pin (PH.08),
   * and a rising edge on this input is used to trigger the Timer.
   *
   *-The One Pulse signal is output on TIM5_CH1 (PI.5).
   *
  ...
  if (HAL_TIM_OnePulse_ConfigChannel(&htim5, &sConfig, TIM_CHANNEL_1, TIM_CHANNEL_2) != HAL_OK)
  {
	/* Configuration Error */
	Error_Handler();
  }
...
/* TIM5 Deinit function */
static void MX_TIM5_DeInit(void)
{
	/* Deinitialize the TIM5 peripheral */
	HAL_TIM_OnePulse_DeInit(&htim5);

	/* Release GPIOH8 using Resource manager */
	ResMgr_Release(RESMGR_RESOURCE_RIF_GPIOH, RESMGR_GPIO_PIN(8));

	/* Release GPIOI5 using Resource manager */
	ResMgr_Release(RESMGR_RESOURCE_RIF_GPIOI, RESMGR_GPIO_PIN(5));
