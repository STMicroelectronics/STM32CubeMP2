...
/**
  ******************************************************************************
  * @file    TIM\TIM_DMABurst\CM33\NonSecure\Src\main.c
...
/* -----------------------------------------------------------------------
    TIM8 Configuration: generate 1 PWM signal using the DMA burst mode:

    TIM8 input clock (TIM8CLK) is a multiple of APB2 clock (PCLK2).
        TIM8CLK is equal to the APB2 Timer Clocks
    => TIM8CLK = HAL_RCCEx_GetTimerCLKFreq(RCC_PERIPHCLK_TIM8)

    To get TIM8 counter clock at 16 MHz, the prescaler is computed as follows:
      Prescaler = (TIM8CLK / TIM8 counter clock) - 1
      Prescaler = (TIM8CLK /16 MHz) - 1

    The TIM8 Frequency = TIM8 counter clock/(ARR + 1)
                       = 16 MHz / 4096 = 3.9 KHz
    TIM8 Channel1 duty cycle = (TIM8_CCR1/ TIM8_ARR)* 100 = 33.33%

    Note:
     SystemCoreClock variable holds HCLK frequency and is defined in system_stm32mp2xx.c file.
...
#ifdef ICACHE_DCACHE_ENABLE
  HAL_DCACHE_CleanByAddr(&hdcache, (uint32_t*)aSRC_Buffer, sizeof(aSRC_Buffer));
#endif /* ICACHE_DCACHE_ENABLE */

  /*## Start DMA Burst transfer ###########################################*/
  HAL_TIM_DMABurst_WriteStart(&htim8, TIM_DMABASE_ARR, TIM_DMA_UPDATE,
...
static void MX_TIM8_Init(void)
{
  ...
  /* Acquire TIM 8 using Resource Manager */
  ...
  /* Acquire GPIO Pin PC10 ---> TIM8_CH4 using Resource Manager */
...
/* TIM8 Deinit function */
static void MX_TIM8_DeInit(void)
{
  /* Deinitialize the TIM peripheral */
  HAL_TIM_PWM_DeInit(&htim8);

  /*Release the HPDMA3, TIM8 and GPIO PC10 using the Resource Manager */
  (void) ResMgr_Release(RESMGR_RESOURCE_RIF_HPDMA3, RESMGR_HPDMA_CHANNEL(1));
  (void) ResMgr_Release(RESMGR_RESOURCE_RIFSC, RESMGR_RIFSC_TIM8_ID);
