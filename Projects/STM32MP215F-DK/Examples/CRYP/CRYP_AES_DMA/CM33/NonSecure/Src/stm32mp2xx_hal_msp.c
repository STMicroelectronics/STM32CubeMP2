  ...
  if(hcryp->Instance==CRYP2)
  {
    ...
    __HAL_RCC_CRYP2_CLK_ENABLE();
	 ...
	 /***************** Configure HPDMA3 IN for CRYP2 ************************/
	  ...
	  hdmaIn.Init.TransferAllocatedPort = DMA_SRC_ALLOCATED_PORT0 | DMA_DEST_ALLOCATED_PORT1;
	  ...
	  {
	    if (HAL_DMA_ConfigChannelAttributes(&hdmaIn, (DMA_CHANNEL_CID_STATIC_2 | DMA_CHANNEL_PRIV | DMA_CHANNEL_SEC | DMA_CHANNEL_DEST_SEC | DMA_CHANNEL_SRC_SEC)) != HAL_OK)
		    {
	      Error_Handler();
	    }
	  }
	  ...
	  hdmaOut.Init.TransferAllocatedPort = DMA_SRC_ALLOCATED_PORT1 | DMA_DEST_ALLOCATED_PORT0;
	  ...
	  if(IS_DEVELOPER_BOOT_MODE())
	  {
		if (HAL_DMA_ConfigChannelAttributes(&hdmaOut, (DMA_CHANNEL_CID_STATIC_2 | DMA_CHANNEL_PRIV | DMA_CHANNEL_SEC | DMA_CHANNEL_DEST_SEC | DMA_CHANNEL_SRC_SEC)) != HAL_OK)
		  {
	      Error_Handler();
	    }
	  }
