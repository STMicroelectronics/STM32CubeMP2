...
static void MX_DMA_Init(void)
{
  ...
  handle_HPDMA3_Channel13.InitLinkedList.LinkAllocatedPort = DMA_LINK_ALLOCATED_PORT0;
  ...
  if(IS_DEVELOPER_BOOT_MODE())
  {
    if (HAL_DMA_ConfigChannelAttributes(&handle_HPDMA3_Channel13, (DMA_CHANNEL_CID_STATIC_2 | DMA_CHANNEL_PRIV | DMA_CHANNEL_SEC | DMA_CHANNEL_DEST_SEC | DMA_CHANNEL_SRC_SEC)) != HAL_OK)
    {
      Error_Handler();
    }
  }
