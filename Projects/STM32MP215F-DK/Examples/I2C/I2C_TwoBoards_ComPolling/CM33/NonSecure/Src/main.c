/**
  ...
  * @brief   This sample code shows how to use STM32MP21xx I2C HAL API to transmit
...
uint32_t status;
  ...
  while(HAL_I2C_Master_Transmit(&hi2c, (uint16_t)I2C_ADDRESS, (uint8_t*)aTxBuffer, TXBUFFERSIZE, 10000)!= HAL_OK)
  {
  ...
  /* Timeout is set to 10S */
  while(HAL_I2C_Master_Receive(&hi2c, (uint16_t)I2C_ADDRESS, (uint8_t *)aRxBuffer, RXBUFFERSIZE, 10000) != HAL_OK)
  {
    /* Error_Handler() function is called when Timeout error occurs.
  ...
  if(HAL_I2C_Slave_Receive(&hi2c, (uint8_t *)aRxBuffer, RXBUFFERSIZE, 10000) != HAL_OK)
  {
  ...
  /* While the I2C in reception process, user can transmit data through
     "aTxBuffer" buffer */
  if(HAL_I2C_Slave_Transmit(&hi2c, (uint8_t*)aTxBuffer, TXBUFFERSIZE, 10000)!= HAL_OK)
  {
