/**
  ...
  * @brief   This sample code shows how to use STM32MP2xx I2C HAL API to transmit
...
uint32_t status;
  ...
  HAL_DCACHE_CleanByAddr(&hdcache, (uint32_t *)aTxBuffer, TXBUFFERSIZE);
#endif /* ICACHE_DCACHE_ENABLE */
  ...
  do
  {
    if(HAL_I2C_Master_Transmit_DMA(&hi2c, (uint16_t)I2C_ADDRESS, (uint8_t*)aTxBuffer, TXBUFFERSIZE)!= HAL_OK)
    {
  ...
  do
  {
    if(HAL_I2C_Master_Receive_DMA(&hi2c, (uint16_t)I2C_ADDRESS, (uint8_t *)aRxBuffer, RXBUFFERSIZE) != HAL_OK)
    {
      /* Error_Handler() function is called when error occurs. */
      Error_Handler();
    }
...
#ifdef ICACHE_DCACHE_ENABLE
  /* Invalidating cache lines after DMA transfer */
    HAL_DCACHE_InvalidateByAddr(&hdcache, (uint32_t *)aRxBuffer, RXBUFFERSIZE);
#endif /* ICACHE_DCACHE_ENABLE */
  ...
  /*##- Put I2C peripheral in reception process ###########################*/
  if(HAL_I2C_Slave_Receive_DMA(&hi2c, (uint8_t *)aRxBuffer, RXBUFFERSIZE) != HAL_OK)
  {
  ...
  /* While the I2C in reception process, user can transmit data through
     "aTxBuffer" buffer */
  if(HAL_I2C_Slave_Transmit_DMA(&hi2c, (uint8_t*)aTxBuffer, TXBUFFERSIZE)!= HAL_OK)
  {
...
static void MX_I2C1_DeInit(void)
{
  ...
  (void) ResMgr_Release(RESMGR_RESOURCE_RIFSC, STM32MP21_RIFSC_I2C1_ID);

  /* Release GPIOA3 using Resource manager */
