...
int main(void)
{
  ...
  printf("Clock 9 gate: Return received from SCMI server = %ld\r\n", g_message_received_flag);
  ...
  printf("Clock 9 config after enable: config = 0x%lx, attributes = 0x%lx, Return received from SCMI server = %ld\r\n",
         config, attributes, g_message_received_flag);
  ...
  printf("Clock 12 config after enable: config = 0x%lx, attributes = 0x%lx, Return received from SCMI server = %ld\r\n",
         config, attributes, g_message_received_flag);
  ...
  printf("Clock 9 gate disable: Return received from SCMI server = %ld\r\n", g_message_received_flag);

  /* API to disable the clock */
  ...
  /* API to get the configuration of the clock having clock_id 7 */
  g_message_received_flag = scmi_clock_get_config(&scmi_channel, 7, &flags, &attributes, &config);
  printf("Clock 7 config after disable: config = 0x%lx, attributes = 0x%lx, Return received from SCMI server = %ld\r\n",
         config, attributes, g_message_received_flag);

  /* API to get the configuration of the clock having clock_id 9 */
  g_message_received_flag = scmi_clock_get_config(&scmi_channel, 9, &flags, &attributes, &config);
  printf("Clock 9 config after disable: config = 0x%lx, attributes = 0x%lx, Return received from SCMI server = %ld\r\n",
         config, attributes, g_message_received_flag);

  /* API to get the configuration of the clock having clock_id 12 */
  g_message_received_flag = scmi_clock_get_config(&scmi_channel, 12, &flags, &attributes, &config);
  printf("Clock 12 config after disable: config = 0x%lx, attributes = 0x%lx, Return received from SCMI server = %ld\r\n",
         config, attributes, g_message_received_flag);
...
static void Get_SCMI_Clock_Attributes(void)
{
  for (int j = 0; j < 15; j++)
  {
    ...
    printf("Clock domain %d: Name = %s, attributes = 0x%lx, Return received from SCMI server = %ld\r\n",
           j, (char *)&clk_name, clk_attributes, g_message_received_flag);
    ...
    printf("Clock domain %d: Frequency (kHz) = %d, Return received from SCMI server = %ld\r\n",
           j, clk_rate_khz, g_message_received_flag);
  }
}
