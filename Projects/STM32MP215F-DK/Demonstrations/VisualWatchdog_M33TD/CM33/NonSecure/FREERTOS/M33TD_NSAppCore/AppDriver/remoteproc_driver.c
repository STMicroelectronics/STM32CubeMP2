...
static int RemoteProcDriver_Init(void)
{
  ...
  /* Acquire EXTI2 line 34 using Resource manager */
  if (RESMGR_STATUS_ACCESS_OK != ResMgr_Request(RESMGR_RESOURCE_RIF_EXTI2, RESMGR_EXTI_RESOURCE(34)))
  {
    Error_Handler();
  }

  /* Set configuration except Interrupt and Event mask of EXTI line 34 */
  EXTI_ConfigStructure.Line = EXTI2_LINE_34;
...
static int RemoteProcDriver_Deinit(void)
{
  /* Release EXTI2 line 34 using Resource manager */
