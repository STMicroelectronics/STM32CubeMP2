...
/* Synchronization check variables */
static volatile uint32_t tim3_last = 0, tim8_last = 0, lptim4_last = 0;
...
{
    // Start TIM3  in interrupt mode (they will wait for TIM8 trigger)
    ...
    // Wait for a few seconds to observe synchronization 
    osDelay(5000);
...
{
    // Acquire TIM3 using the resource manager
    if (RESMGR_STATUS_ACCESS_OK != ResMgr_Request(RESMGR_RESOURCE_RIFSC, STM32MP21_RIFSC_TIM3_ID)) {
        return TEST_FAIL;
    }

    // Acquire TIM8 using the resource manager
    if (RESMGR_STATUS_ACCESS_OK != ResMgr_Request(RESMGR_RESOURCE_RIFSC, STM32MP21_RIFSC_TIM8_ID)) {
        return TEST_FAIL;
    }

    // Acquire LPTIM4 using the resource manager
    if (RESMGR_STATUS_ACCESS_OK != ResMgr_Request(RESMGR_RESOURCE_RIFSC, STM32MP21_RIFSC_LPTIM4_ID)) {
        return TEST_FAIL;
    }
...
TEST_STATUS Test_TIMER_ResourceRelease(void)
{
    ...
    // Release TIM3 using the resource manager
    if (RESMGR_STATUS_ACCESS_OK != ResMgr_Release(RESMGR_RESOURCE_RIFSC, STM32MP21_RIFSC_TIM3_ID)) {
        return TEST_FAIL;
    }

    // Release TIM8 using the resource manager
    if (RESMGR_STATUS_ACCESS_OK != ResMgr_Release(RESMGR_RESOURCE_RIFSC, STM32MP21_RIFSC_TIM8_ID)) {
        return TEST_FAIL;
    }

    // Release LPTIM4 using the resource manager
    if (RESMGR_STATUS_ACCESS_OK != ResMgr_Release(RESMGR_RESOURCE_RIFSC, STM32MP21_RIFSC_LPTIM4_ID)) {
        return TEST_FAIL;
    }
...
/**
  * @}
  */
#endif /* ENABLE_TIMERS_TEST */  // End of conditional compilation
