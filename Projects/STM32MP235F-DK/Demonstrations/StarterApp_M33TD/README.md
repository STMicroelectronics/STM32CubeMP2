...
In the STM32MP235F-DK StarterApp profile, `NSCoreApp_Init()` starts the enabled utility tasks below:
...
- **Supported Devices**: This example runs on STM32MP23xx devices and has been tested with the STMicroelectronics STM32MP235F-DK board. It can be tailored to other supported devices and development boards.
...
2. For SD card development mode, execute the following command:
   ```bash
   cmake -B config_default -G"Unix Makefiles" -DTFM_PLATFORM=stm/stm32mp235f_dk -DTFM_TOOLCHAIN_FILE=toolchain_GNUARM.cmake -DSTM32_BOOT_DEV=sdmmc1 -DTFM_PROFILE=profile_medium -DSTM32_M33TDCID=ON -DCMAKE_BUILD_TYPE=Relwithdebinfo -DNS=OFF -DDTS_EXT_DIR=<EXT_DT_DIR> -DDTS_BOARD_BL2=stm32mp2/m33-td/mcuboot/stm32mp235f-dk-cm33tdcid-ostl-sdcard-bl2.dts -DDTS_BOARD_S=stm32mp2/m33-td/tfm/stm32mp235f-dk-cm33tdcid-ostl-sdcard-s.dts 
   -DDTS_BOARD_NS=stm32mp2/m33-td/tfm/stm32mp235f-dk-cm33tdcid-ostl-ns.dts 
   ```
3. Build the project:
   ```bash
...
1. Navigate to `Firmware/Projects/STM32MP235F-DK/Demonstrations/StarterApp_M33TD`.
...
1. Configure the CMake build options:
    - Navigate to `StarterApp_M33TD_CM33_trusted-firmware-m` and update the CMake settings:
      ...
      -DTFM_PLATFORM=stm/stm32mp235f-dk
      ...
      -DDTS_BOARD_BL2=stm32mp2/m33-td/mcuboot/stm32mp235f-dk-cm33tdcid-ostl-sdcard-bl2.dts  # External DT file for sdcard_sdcard bootdevice mode 
      -DDTS_BOARD_S=stm32mp2/m33-td/tfm/stm32mp235f-dk-cm33tdcid-ostl-sdcard-s.dts          # External DT file for sdcard_sdcard bootdevice mode 
      -DDTS_BOARD_NS=stm32mp2/m33-td/tfm/stm32mp235f-dk-cm33tdcid-ostl-ns.dts               # External DT file common for all bootdevice modes
      ```
...
3. **Modify Flashlayout (TSV)**  
   Select the relevant TSV file under `<OSTL-IMAGE-PATH>/images/stm32mp2-m33td/flashlayout_st-image-weston/optee` as per specific dev mode (see [Flash Layouts for MP23-DK Board](#flash-layouts-for-mp23-dk-board)).
...
## Different Boot Device Modes for MP23-DK Board

The MP23-DK board supports the following boot device modes:
...
## Flash Layouts for MP23-DK Board

For the MP23-DK board, five TSV flavors are provided to flash under the M33TDCID profile:

- `FlashLayout_sdcard_stm32mp235f-dk-cm33tdcid-ostl-optee.tsv`

**Note**: Each TSV requires three sets of binaries. Refer to the [Reference Use Cases for MP23-DK Board (StarterApp Project)](#reference-use-cases-for-mp23-dk-board-starterapp-project) section for details.
...
1. **Configure and Build TFM Secure**:  
  Use specific CMake options defined for each boot mode as described in the **Required CMake Options** section under [Reference Use Cases for MP23-DK Board (StarterApp Project)](#reference-use-cases-for-mp23-dk-board-starterapp-project).  
...
3. **Rename Generated Binaries**:  
   Rename the binaries in the `bin/` folder to match the required binaries for the specific TSV as described in the **Required Binaries** section under [Reference Use Cases for MP23-DK Board (StarterApp Project)](#reference-use-cases-for-mp23-dk-board-starterapp-project).  

---

## Reference Use Cases for MP23-DK Board (StarterApp Project)

### 1. FlashLayout_sdcard_stm32mp235f-dk-cm33tdcid-ostl-optee.tsv (sdcard_sdcard Boot Mode)

**Required Binaries**:  
- `bl2-stm32mp235f-dk-cm33tdcid-ostl-sdcard.stm32`  
- `ddr_phy-stm32mp235f-dk-cm33tdcid-ostl-sdcard_Signed.bin`  
- `tfm-starterapp-stm32mp235f-dk-cm33tdcid-ostl-sdcard-sdcard_s_ns_Signed.bin`  

**Required CMake Options**:  
```bash
-DSTM32_BOOT_DEV=sdmmc1  # SD card
-DDTS_BOARD_BL2=stm32mp2/m33-td/mcuboot/stm32mp235f-dk-cm33tdcid-ostl-sdcard-bl2.dts
-DDTS_BOARD_S=stm32mp2/m33-td/tfm/stm32mp235f-dk-cm33tdcid-ostl-sdcard-s.dts
-DDTS_BOARD_NS=stm32mp2/m33-td/tfm/stm32mp235f-dk-cm33tdcid-ostl-ns.dts
```
