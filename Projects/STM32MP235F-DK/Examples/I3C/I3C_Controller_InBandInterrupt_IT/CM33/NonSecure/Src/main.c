...
static void MX_I3C_DeInit(void);
...
int main(void)
{
  ...
  if (HAL_I3C_Ctrl_DynAddrAssign_IT(&hi3c4, I3C_ONLY_ENTDAA) != HAL_OK)
  {
  ...
  for(ubTargetIndex = 0; ubTargetIndex < uwTargetCount; ubTargetIndex++)
  {
    ...
    if (HAL_I3C_Ctrl_ConfigBusDevices(&hi3c4, &DeviceConf[ubTargetIndex], 1U) != HAL_OK)
    {
      Error_Handler();
    }
  }
  ...
  if( HAL_I3C_ActivateNotification(&hi3c4, NULL, HAL_I3C_IT_IBIIE) != HAL_OK)
  {
    /* Error_Handler() function is called when error occurs. */
    Error_Handler();
  }
  ...
  while (1)
  {
	...
	if (HAL_I3C_GetCCCInfo(&hi3c4, EVENT_ID_IBI, &CCCInfo) != HAL_OK)
	{
	  /* Error_Handler() function is called when error occurs. */
	  Error_Handler();
	}
...
static void MX_I3C4_Init(void)
{
  ...
  if (RESMGR_STATUS_ACCESS_OK != ResMgr_Request(RESMGR_RESOURCE_RIFSC, STM32MP23_RIFSC_I3C4_ID))
  {
	  Error_Handler();
  }
  ...
  hi3c4.Init.CtrlBusCharacteristic.SCLODLowDuration = 0x59;
  hi3c4.Init.CtrlBusCharacteristic.SCLI2CHighDuration = 0x00;
  hi3c4.Init.CtrlBusCharacteristic.BusFreeDuration = 0x32;
...
static void MX_I3C_DeInit(void)
{
	...
	ResMgr_Release(RESMGR_RESOURCE_RIFSC, STM32MP23_RIFSC_I3C4_ID);
}
...
{
  /* Deinitialize the peripheral and resources*/
  MX_I3C_DeInit();
