...
#endif
/* Comment this line to use the board as master, if not it is used as slave */
#define MASTER_BOARD
...
static void MX_I2C8_Init(void)
{
	if (RESMGR_STATUS_ACCESS_OK != ResMgr_Request(RESMGR_RESOURCE_RIFSC, STM32MP23_RIFSC_I2C8_ID))
	{
	  Error_Handler();
	}
...
static void MX_I2C8_DeInit(void)
{
  ...
  ResMgr_Release(RESMGR_RESOURCE_RIFSC, STM32MP23_RIFSC_I2C8_ID);
